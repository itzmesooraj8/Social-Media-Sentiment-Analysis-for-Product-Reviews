...
export const LiveReviewAnalyzer: React.FC = () => {
  ...
  const runAnalyze = async () => {
    if (!text.trim()) {
      toast({ title: 'Enter text to analyze' });
      return;
    }
    ...
    try {
      ...
      // Expected shape: { label, score, emotion, credibility }
      setResult(res);
    } catch (e) {
  ...
  const badgeColor = (label?: string) => {
    ...
    if (l.includes('pos')) return 'bg-green-100 text-green-800';
    if (l.includes('neg')) return 'bg-red-100 text-red-800';
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Live Review Analyzer</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <Textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Paste review text or type something to analyze..." />
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="secondary" onClick={() => { setText(''); setResult(null); }}>
              Clear
            </Button>
          </div>
            ...
            <div className="border rounded p-3 bg-background">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Badge className={badgeColor(result?.label)} variant="outline">{(result?.label || 'NEUTRAL').toString().toUpperCase()}</Badge>
                  <div className="text-sm text-muted-foreground">Confidence: <span className="font-medium">{Math.round((result?.score || 0) * 100)}%</span></div>
                </div>
                <div className="text-sm text-muted-foreground">Emotion: <span className="font-medium">{result?.emotion || 'neutral'}</span></div>
              </div>

              {result?.explanation && (
                <div className="mt-3 text-sm text-muted-foreground">{result.explanation}</div>
              )}
            </div>
          )}
        </div>
...
export default LiveReviewAnalyzer;
